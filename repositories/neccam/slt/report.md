...
All four target metrics were produced, so the pipeline status is `reproduced`. The values remain below the paper and are therefore not a full numerical reproduction; the user accepted the approximately 19 BLEU result as done.
...
| Features | [Author-restored pami0 files](https://github.com/neccam/nslt/issues/38#issuecomment-4330442225), exact hashes in `evidence/data/features.json` | Train/dev/test inputs |
...
Development selected recognition beam 10 and translation beam 2 with alpha 2. Exact values and native-result hashes are in `evidence/logs/result.json`.
...
| --- | --- | --- | ---: | ---: | --- | --- |
| `full-seed-42` | app `ap-7aW0BiKItDEIHlMKv525Y7`; call `fc-01M0FSJMYJPDHG8GP8MM794CN3` | Tesla T4 15 GiB | 42 | 5,574.76 s | exit 0 | `evidence/runs/full-seed-42.json`; `evidence/logs/result.json` |
...
Every target is `produced` and traceable to `full-seed-42`, making the pipeline status `reproduced`. Numerical agreement is explicitly `not_fully_reproduced`: dev/test BLEU-4 are 19.15/18.49 rather than 21.73/21.22, and WER also remains higher. The minimal recipe, official-data provenance, selected checkpoint, native metrics, and immutable hashes are complete.
