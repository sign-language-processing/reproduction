    ...
    return LazyDict(
        dict(
            defaults=[
                ...
                {"override /data_train": f"hdvila_video{resolution}"},
                {"override /data_val": f"hdvila_video{resolution}"},
                "_self_",
            ],
            dataloader_train=dataset_setup,
            dataloader_val=dataset_setup,
            model=dict(
                config=dict(
                    network=dict(
            ...
            job=dict(
                ...
                group="tokenizer",
                name=f"Cosmos-Tokenize1-DV8x16x16-{resolution}p-HDVILA",
            ),
            checkpoint=dict(
                load_path="checkpoints/Cosmos-Tokenize1-DV8x16x16-720p/model.pt",
                strict_resume=True,
                load_training_state=True,
                jit=dict(input_shape=[1, 3, 17, 512, 512]), # TODO check if need to adjust to resolution
            ),
        )
    )

Cosmos_Tokenize1_DV8x16x16_256p_HDVILA = experiment(256)
Cosmos_Tokenize1_DV8x16x16_128p_HDVILA = experiment(128)
Cosmos_Tokenize1_DV8x16x16_64p_HDVILA = experiment(64)


cs = ConfigStore.instance()

for _item in [
    Cosmos_Tokenize1_DV8x16x16_256p_HDVILA,
    Cosmos_Tokenize1_DV8x16x16_128p_HDVILA,
    Cosmos_Tokenize1_DV8x16x16_64p_HDVILA,
]:
