# Stage 2 Hybrid Tokenizer — 384x384

Stage 2 fine-tunes the hybrid tokenizer jointly with LoRA adapters on the LTX-2.3 VAE encoder and decoder, using pixel-space losses. Initialized from `s1_hybrid_384/best.pt`. See [sign/ltx-vae-quantizer](https://github.com/sign/ltx-vae-quantizer) for the repo and CLI.
...
|-------------------------|---------------|
| Wall time               | ~15h 30m      |
| Step throughput         | ~5.2 s/step   |
| Total steps             | 10,760 (stopped manually) |
| Best step (saved)       | 10,600        |
| Best val LPIPS          | **0.0229**    |
| Val PSNR at best LPIPS  | 30.52         |
| Stage 1 baseline LPIPS  | 0.0571        |
| Stage 1 baseline PSNR   | 29.91         |

Stage 2 cuts LPIPS by ~2.5× from the stage 1 init (0.0571 → 0.0229) and raises PSNR by ~0.6 dB. The run kept finding marginal new bests every few hundred steps without triggering the 10-check early-stop, so it was stopped manually after ~15.5 hours. See `metrics.json` for the full per-step trajectory and `val_comparison.mp4` for the rendered side-by-side.
...
docker run --rm --gpus all \
  ...
  -v /path/to/videos_384:/workspace/videos:ro \
  -v $(pwd):/workspace/output \
  -v /path/to/vae.safetensors:/workspace/vae.safetensors:ro \
  -v /path/to/s1_hybrid_384/best.pt:/workspace/tokenizer.pt:ro \
  ltx-vae-quantizer \
